...
angular.module('paging', []).directive('paging', function () {
    ...
    function fieldLink(scope, el, attrs) {
        ...
        scope.$watchCollection('[page,pageSize,total]', function (old,newVal) {
            build(scope, attrs);
        });
    }
    ...
    function fieldTemplate(el, attrs){
            ...
            //     '</li>' +
            // '</ul>'
        return  '<div class="mt-10 page f-r clearfix" data-ng-hide="Hide" data-ng-class="ulClass">' +
                    '<div class="pagination">' +
                        // '<span class="current prev"> 上一页 </span>' +
                        '<a title="{{Item.title}}" data-ng-repeat="Item in List"' +
                            (attrs.pgHref ? 'data-ng-href="{{Item.pgHref}}" ' : 'href ') +
                            'data-ng-class="Item.liClass" ' +
                            'data-ng-click="Item.action()" ' +
                            'data-ng-bind="Item.value">'+
                        '</a>' +
                        // '<a class="next" data-ng-click="Item.nextPrev()" >下一页 </a>' +
                        '<div class="page-info">共{{total}}条，共{{pageCount}}页</div>' +
                        // '<input type="text" value="1" class="jumpipt jumpPage " />' +
